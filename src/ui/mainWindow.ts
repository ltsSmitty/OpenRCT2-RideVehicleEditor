import { box, button, checkbox, CheckboxParams, colourPicker, compute, dropdown, dropdownSpinner, FlexiblePosition, groupbox, horizontal, label, listview, LabelParams, SpinnerParams, SpinnerWrapMode, toggle, vertical, viewport, WidgetCreator, window } from "openrct2-flexui";
import { isDevelopment, pluginVersion } from "../environment";
import { combinedLabelSpinner, combinedLabelCheckbox } from "./utilityControls";
...
let title = `Matching Cars & Tracks (v${pluginVersion})`;
if (isDevelopment) {
	title += " [DEBUG]";
}
...
export const mainWindow = (model: RideViewModel) => window({
	...
	content: [
		...
		listview({
			items: compute(model.ridesToPaint, r => r.map(r => [`${r.ride.ride().name}`, `${r.values.enableColourMatching}`, `${r.values.enableColourReset}`])),
			columns: [{ header: "Id" }, { header: "Colour matching" }, { header: "Colour reset" }],
			height: 100,
		})
